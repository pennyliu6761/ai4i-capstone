...
from pages.loader import (load_bundle, FAILURE_LABELS, FAILURE_LONG,
                           ML_MODELS, DL_MODELS, mcolor, sec, plotly_base, kpi_card)
...
def stage1_check(wear, air_t, proc_t, rpm, torque, m_type):
    ...
    import math
    alerts      = []
    power_w     = rpm * 2 * math.pi / 60 * torque
    temp_diff   = proc_t - air_t
...
def show():
    ...
    scaler    = b['scaler']
    models    = b['models']
    FEAT_COLS = b['feat_cols']   # 與 scaler 訓練時一致，不寫死
    ...
    # 主結果（LightGBM，Stage 2 最佳模型）
    primary = 'LightGBM'
    if primary not in preds_all:
        primary = list(preds_all.keys())[0]
    p_cls, p_prob = preds_all[primary]
    ...
    <div class="kpi-card" style="text-align:center;padding:1.4rem;margin-bottom:1.2rem;
        ...
        <div style="color:#8888aa;font-size:.82rem">
            {primary}（Stage 2 最佳模型）· 類別 {p_cls} · 信心度
        </div>
    ...
    for i, (name, (cls, prob)) in enumerate(preds_all.items()):
        ...
        cols_g[i % 4].markdown(f"""
        <div class="kpi-card" style="padding:.8rem 1rem;border-left:3px solid {mc}">
            <div style="display:flex;justify-content:space-between;margin-bottom:.2rem">
                <span style="font-size:.68rem;color:#8888aa">{name}</span>
                <span style="font-size:.65rem;color:{mc}">[{tag}]</span>
            </div>
            <div style="display:flex;align-items:center;gap:.5rem">
                <span style="font-size:1.3rem">{em}</span>
                <div>
                    <div style="font-weight:600;color:#e0e0ff;font-size:.83rem">{fl}</div>
                    <div style="font-family:monospace;color:{mc};font-size:.76rem">
                        {cf:.1f}% 信心
                    </div>
                </div>
            </div>
    ...
    <div class="kpi-card" style="padding:.9rem 1.2rem">
        <div class="kpi-lbl">🗳️ 模型投票共識（{len(preds_all)} 個模型）</div>
