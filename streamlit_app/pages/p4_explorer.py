...
def load_raw():
    ...
    feat   = b['feat_cols']
    df     = pd.DataFrame(X_orig, columns=feat)
...
def show():
    ...
    with t1:
        ...
        # SMOTE 後各類別數（來自 Cell 5a 結果：7721 筆）
        smote_after = {0: 7721, 1: 7721, 2: 7721, 3: 7721}
        c1, c2 = st.columns(2)
        for col, (title, data) in zip([c1, c2], [
            ...
            fig.add_annotation(text=f'<b>{sum(vals):,}</b><br>筆',
                ...
                font=dict(size=13, color='#c0c8ff'), align='center')
    ...
    with t2:
        ...
        sel = st.selectbox("選擇特徵", num_feats,
            format_func=lambda x: FEAT_ZH_SAFE.get(x, x), index=5)
        fig = go.Figure()
        ...
        fig.update_layout(**plotly_base(
            height=400,
            yaxis=dict(title=FEAT_ZH_SAFE.get(sel, sel), gridcolor='#2a2a4a'),
        ...
        st.dataframe(
            df.groupby('故障類別')[sel].describe()[['mean', 'std', 'min', '50%', 'max']]
              ...
              .rename(columns={'mean': '平均', 'std': '標準差',
                               'min': '最小', '50%': '中位數', 'max': '最大'})
              .style.format('{:.2f}'),
    ...
    with t3:
        ...
        corr.index = corr.columns = [FEAT_ZH_SAFE.get(c, c) for c in corr.index]
    ...
    with t4:
        ...
        cards = [
            ...
            (1, '散熱不良 HDF', '🌡️', '#f97316',
             ...
             '106', '溫差<8.6K 且 轉速<1380rpm', '溫差（SHAP=4.141）'),
            (2, '功率異常 PWF', '⚡', '#fbbf24',
             '轉速×扭矩（功率）超出正常範圍 [3,500, 9,000] W 時觸發。',
             '83', '功率<3500W 或 >9000W', '功率（SHAP=4.011）'),
            (3, '過負荷 OSF',   '💪', '#a78bfa',
             '功率×磨耗超過型別門檻：L型 11,000、M型 12,000、H型 13,000 Nm·min。',
             '95', '功率×磨耗超門檻', '功率×磨耗（SHAP=4.257）'),
        ]
        cl, cr = st.columns(2)
